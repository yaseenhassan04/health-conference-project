...
function SessionCard({ session }) {
  ...
  return (
    <div style={{ display:'flex', background:'#fff', border:`1px solid ${isOpening ? G+'60' : '#e8edf2'}`, borderRadius:14, marginBottom:10, overflow:'hidden', transition:'all .25s', opacity: isBreak ? 0.7 : 1 }}
      onMouseEnter={e => { if (!isBreak) { e.currentTarget.style.borderColor = G; e.currentTarget.style.transform = 'translateX(-3px)'; e.currentTarget.style.boxShadow = '0 8px 24px rgba(27,54,93,0.08)'; }}}
      onMouseLeave={e => { e.currentTarget.style.borderColor = isOpening ? G+'60' : '#e8edf2'; e.currentTarget.style.transform = 'translateX(0)'; e.currentTarget.style.boxShadow = 'none'; }}>
      <div style={{ minWidth:110, padding:'16px 12px', borderLeft:`1px solid #e8edf2`, background: isOpening ? 'rgba(212,175,55,0.06)' : isBreak ? 'rgba(27,54,93,0.02)' : 'rgba(27,54,93,0.03)', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:4, flexShrink:0 }}>
        <div style={{ fontFamily:'monospace', fontSize:12, fontWeight:700, color: isOpening ? G : isBreak ? '#94a3b8' : B, direction:'ltr', textAlign:'center' }}>{session.timeStart}</div>
        <div style={{ fontSize:10, color:'#cbd5e1' }}>↕</div>
        <div style={{ fontFamily:'monospace', fontSize:12, fontWeight:700, color: isOpening ? G : isBreak ? '#94a3b8' : B, direction:'ltr', textAlign:'center' }}>{session.timeEnd}</div>
      </div>
      <div style={{ padding:'16px 20px', flex:1 }}>
        <div style={{ fontSize: isBreak ? 13 : 15, fontWeight:700, color: isBreak ? '#94a3b8' : B, marginBottom: session.desc ? 6 : 0, fontStyle: isBreak ? 'italic' : 'normal', display:'flex', alignItems:'center', gap:8 }}>
          {isOpening && <span style={{ fontSize:16 }}>⭐</span>}
          {isBreak && <span style={{ fontSize:14 }}>☕</span>}
          {session.name}
        </div>
        {session.desc && <div style={{ fontSize:13, color:'#64748b', lineHeight:1.6, marginBottom:8 }}>{session.desc}</div>}
        {session.tags.length > 0 && (
          <div style={{ display:'flex', flexWrap:'wrap', gap:6 }}>
            {session.tags.map((tag, i) => (
              <span key={i} style={{ fontSize:11, padding:'3px 10px', borderRadius:20, background: isOpening ? 'rgba(212,175,55,0.1)' : 'rgba(27,54,93,0.06)', border:`1px solid ${isOpening ? G+'40' : 'rgba(27,54,93,0.1)'}`, color: isOpening ? '#92700a' : '#64748b', fontWeight:600 }}>{tag}</span>
            ))}
          </div>
        )}
      </div>
      {!isBreak && <div style={{ width:4, background: isOpening ? `linear-gradient(180deg,${G},${G}80)` : `linear-gradient(180deg,${B},${R})`, flexShrink:0 }} />}
    </div>
  );
}
...
export default function Program() {
  ...
  return (
    <div style={{ direction: isRTL ? 'rtl' : 'ltr', fontFamily:"'Cairo',sans-serif", background:'#F4F7F9', minHeight:'100vh' }}>
      ...
      {/* NAV */}
      <nav style={{ height:56, background:'#fff', borderBottom:'1px solid #e8edf2', display:'flex', alignItems:'center', justifyContent:'flex-end', padding:'0 28px', position:'sticky', top:0, zIndex:200 }}>
        <button onClick={() => setLang(lang === 'ar' ? 'en' : 'ar')}
          style={{ padding:'6px 18px', border:`1.5px solid ${B}`, borderRadius:50, background:'#fff', color:B, fontFamily:"'Cairo',sans-serif", fontSize:13, fontWeight:700, cursor:'pointer' }}>
          🌐 {lang === 'ar' ? 'English' : 'العربية'}
        </button>
      </nav>

      {/* ECG */}
      <div style={{ height:52, background:'rgba(27,54,93,0.03)', borderBottom:'1px solid rgba(27,54,93,0.07)', overflow:'hidden' }}>
        <svg width="100%" height="52" viewBox="0 0 1200 52" preserveAspectRatio="none">
          <path className="ecg-line" d="M0 26 L80 26 L95 10 L110 42 L125 26 L250 26 L265 5 L285 47 L300 26 L500 26 L515 10 L530 42 L545 26 L700 26 L715 5 L735 47 L750 26 L900 26 L915 10 L930 42 L945 26 L1200 26" fill="none" stroke={R} strokeWidth="2.2"/>
        </svg>
      </div>
      ...
      <div style={{ maxWidth:860, margin:'0 auto', padding:'36px 20px 60px' }}>
        ...
        <div style={{ textAlign:'center', marginBottom:40 }}>
          <div style={{ display:'inline-flex', alignItems:'center', gap:8, background:'rgba(200,16,46,0.07)', border:'1px solid rgba(200,16,46,0.2)', borderRadius:50, padding:'5px 16px', fontSize:12, fontWeight:700, color:R, marginBottom:16 }}>
          ...
          <h1 style={{ fontSize:32, fontWeight:900, color:B, margin:'0 0 10px', lineHeight:1.2 }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:10, justifyContent:'center', marginBottom:10 }}>
            <div style={{ width:60, height:2, background:`linear-gradient(90deg,transparent,${G})` }}></div>
            <span style={{ color:R, fontSize:16 }}>♥</span>
            <div style={{ width:60, height:2, background:`linear-gradient(90deg,${G},transparent)` }}></div>
          </div>
          <p style={{ fontSize:14, color:'#64748b', fontWeight:500 }}>
            {lang === 'ar' ? 'جدول الجلسات والمحاضرات — 4-5 ديسمبر 2026 | مجمع ميران، غزة' : 'Session Schedule — December 4-5, 2026 | Miran Complex, Gaza'}
          </p>
        </div>

        {/* PROGRAM DAYS */}
        {program.map((day, di) => (
          <div key={di} className="day-card" style={{ marginBottom:32 }}>
            <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:16 }}>
              <div style={{ background:B, color:G, padding:'4px 12px', borderRadius:8, fontSize:10, fontWeight:700, fontFamily:'monospace', letterSpacing:'0.1em', flexShrink:0 }}>
                {day.label}
              </div>
              <div>
                <div style={{ fontSize:17, fontWeight:900, color:B, lineHeight:1.2 }}>{day.day}: {day.subtitle}</div>
                <div style={{ fontSize:12, color:'#94a3b8', marginTop:2 }}>📅 {day.date}</div>
              </div>
              <div style={{ flex:1, height:1, background:`linear-gradient(${isRTL ? '270deg' : '90deg'},${G}80,transparent)` }}></div>
            </div>
            <div style={{ background:'#fff', borderRadius:16, border:'1px solid #e8edf2', overflow:'hidden', padding:16 }}>
              {day.sessions.map((s, si) => <SessionCard key={si} session={s} />)}
            </div>
          </div>
        ))}

        {/* LIBRARY PANEL */}
        <div style={{ background:'#fff', border:'1px solid #e8edf2', borderRadius:20, padding:32, marginTop:8, position:'relative', overflow:'hidden' }}>
          <div style={{ position:'absolute', top:0, left:0, right:0, height:3, background:`linear-gradient(90deg,${R},${G},${B})` }} />

          <div style={{ display:'flex', alignItems:'center', gap:14, marginBottom:16 }}>
            <div style={{ width:46, height:46, borderRadius:12, background:'rgba(212,175,55,0.1)', border:'1px solid rgba(212,175,55,0.3)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:22 }}>📚</div>
            <div>
          ...
          {loadingLib ? (
            ...
            <div style={{ background:'#F8FAFC', border:'1px dashed #e2e8f0', borderRadius:12, padding:28, textAlign:'center' }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
              {acceptedAbstracts.map((ab, i) => (
                <div key={i} className="lib-item" style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'14px 18px', background:'#F8FAFC', border:`1px solid rgba(212,175,55,0.25)`, borderRight:`4px solid ${G}`, borderRadius:12, transition:'all .25s', gap:16 }}>
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ fontSize:14, fontWeight:700, color:B, marginBottom:4, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
                      📄 {ab.title}
                    </div>
                  ...
                  <a href={ab.filepath || ab.pdfUrl} target="_blank" rel="noreferrer"
                    style={{ padding:'8px 18px', background:B, color:'#fff', borderRadius:8, fontSize:12, fontWeight:700, textDecoration:'none', whiteSpace:'nowrap', flexShrink:0, transition:'all .2s' }}
        ...
        <div style={{ textAlign:'center', marginTop:40 }}>
          <Link href="/participation" style={{ padding:'13px 32px', background:R, color:'#fff', borderRadius:12, fontFamily:"'Cairo',sans-serif", fontSize:15, fontWeight:700, textDecoration:'none', display:'inline-flex', alignItems:'center', gap:8, boxShadow:'0 4px 14px rgba(200,16,46,0.2)' }}>
      ...
      {/* FOOTER */}
      <footer style={{ padding:'28px 40px', borderTop:'1px solid #e8edf2', textAlign:'center', color:'#94a3b8', fontSize:13, background:'#fff' }}>
